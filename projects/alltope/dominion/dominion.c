...
int smithyCard()
{
      ...
      for (i = 0; i < 3; i++)
	{
      ...
      discardCard(handPos, currentPlayer + 1, state, 0); //SMITHY BUG : DISCARDS CARD INTO NEXT PLAYER'S DISCARD PILE
      return 0;
  }
...
int villageCard()
{
      ...
      //+2 Actions
      state->numActions = state->numActions + 1; // VILLAGE BUG : GIVES ONE ACTION INSTEAD OF TWO
	...
	while(card_not_discarded){
	  if (state->hand[currentPlayer][p] == estate){//Found an estate card!
	    ...
	    if (supplyCount(estate, state) > 0){
	      ...
	      state->supplyCount[estate]++;//Decrement estates  //BARON BUG : ADD AN ESTATE INSTEAD OF REDUCING ESTATES BUY ONE
	      if (supplyCount(estate, state) == 0){
		isGameOver(state);
	      }
	    }
